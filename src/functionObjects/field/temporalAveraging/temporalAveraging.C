...
{
    defineTypeNameAndDebug(temporalAveraging, 0);
    addToRunTimeSelectionTable(functionObject, temporalAveraging, dictionary);
}
}
...
void Foam::functionObjects::temporalAveraging::resetFields()
{
    forAll(faItems_, i)
    {
        if (faItems_[i].mean())
...
void Foam::functionObjects::temporalAveraging::restart()
{
...
void Foam::functionObjects::temporalAveraging::calcAverages()
{
    if (!initialised_)
    {
        initialize();
...
void Foam::functionObjects::temporalAveraging::writeAverages() const
{
...
void Foam::functionObjects::temporalAveraging::writeAveragingProperties() const
{
...
void Foam::functionObjects::temporalAveraging::readAveragingProperties()
{
...
Foam::functionObjects::temporalAveraging::temporalAveraging
(
    const word& name,
    const Time& runTime,
...
bool Foam::functionObjects::temporalAveraging::read(const dictionary& dict)
{
...
bool Foam::functionObjects::temporalAveraging::execute()
{
    calcAverages();
